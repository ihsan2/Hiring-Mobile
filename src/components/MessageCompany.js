...
import {getEngineerChat} from '../public/redux/actions/engineerchat';

const heightscr = Dimensions.get('window').height;

class MessageCompany extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      name: '',
  ...
  onSearchCom = async searchKey => {
    if (!searchKey) {
      ...
      try {
        let url = `${API_KEY_URL}/engineers?searchValue=${searchKey}`;
        await this.props.get(url);
      ...
      <View
        ...
        <View
          ...
          <View
            ...
            {this.props.engineerchat.isLoading && (
              <ActivityIndicator
                style={{
                  top: hp('30%'),
                }}></ActivityIndicator>
            ...
            {this.state.isSearch && (
              <FlatList
                ...
                data={this.props.engineerchat.engineerchat}
                renderItem={({item}) => <ListEnToChat itemen={item} />}
...
const mapStateToProps = state => {
  return {
    engineerchat: state.engineerchat,
  };
};
const mapDispatchToProps = dispatch => ({
  get: url => dispatch(getEngineerChat(url)),
});

export default withNavigation(
  connect(mapStateToProps, mapDispatchToProps)(MessageCompany),
);
