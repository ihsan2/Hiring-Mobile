...
const ListEnToChat = props => {
  const {itemen, navigation} = props;
  return (
    <View>
      <List>
        <ListItem thumbnail>
          <Left>
            <Thumbnail
              circular
              source={{uri: `${API_KEY_IMAGE}/engineer/${itemen.image}`}}
            />
          </Left>
          <Body
            style={{
...
export default withNavigation(ListEnToChat);
